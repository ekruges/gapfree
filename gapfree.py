...
def backfill(cfg, start, end, before_creation=False):
    ...
    with _work:
        ...
        while d.isoformat() <= end and d.isoformat() < today:
            ...
            if not (cals[d.year].get(ds, 0) > 0 and ds not in cfg["overrides"]):  # any activity that day: skip
...
def state(cfg, year):
    ...
    for d in year_days(year):
        ...
        plan = day_plan(cfg, d)
        want = len(plan["commits"]) if plan else 0
        p = 0 if (d < today and (r > 0 or o > 0) and d not in cfg["overrides"]) else max(0, want - o)
...
def range_info(cfg, start, end):
    ...
    while d.isoformat() <= end and d.isoformat() < today:
        ...
        want = len(plan["commits"]) if plan else 0
        if want > mine.get(ds, 0) and not (cals[d.year].get(ds, 0) > 0 and ds not in cfg["overrides"]):
            days += 1
...
def publish(body):
    ...
    with _work:
        ensure_repo(CFG)
    _cal.clear()
    return {"repo": CFG["repo"], "commits": sum(ours().values())}
...
<section class="card" id="graphcard">
 ...
 <p class="hint">Backfill pushes the planned commits for every still-empty past day in the selection, dated to their planned minutes. Days with anything on them already are skipped. Issues and PRs cannot be backdated, so past days get commits only.</p>
</section>
